...
public  class MyAspect {
  ...
  public static Set<Long> timeSetLong = ConcurrentHashMap.newKeySet();
    ...
    public MyAspect(CollectorRegistry collectorRegistry) {
  ...
  @Before("execution(* com.example.anycliptest.service.DefaultHandlerImpl.putToFile(..))")
   public void beforeService(JoinPoint joinPoint){
    ...
    @After("execution(* com.example.anycliptest.service.DefaultHandlerImpl.putToFile(..))")
    ...
    @Before("execution(* com.example.anycliptest.service.LogHandlerEnabledTrue.writeToFile(..))")
    ...
    @After("execution(* com.example.anycliptest.service.LogHandlerEnabledTrue.writeToFile(..))")
    ...
    @Before("execution(* com.example.anycliptest.controller.LogRequest.putToFile(..))")
    public void before(JoinPoint joinPoint){
        startProcess = System.currentTimeMillis();
    log.info(joinPoint.getThis().toString()+" Aspect before");
    }

    @AfterReturning(pointcut = "execution(* com.example.anycliptest.controller.LogRequest.putToFile(..))",
    returning = "result")
    public void afterReturn(JoinPoint joinPoint, Response result){
        requestCount.inc();
        log.info(joinPoint.getThis().toString()+" Aspect afterReturn, counter: "+requestCount.get());
    }

    @After("execution(* com.example.anycliptest.controller.LogRequest.putToFile(..))")
